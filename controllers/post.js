...
const { enviarGmail, uploadImagen, eliminarImagen } = require("../helpers/dtmcloud");
...
const deleteItem = async (req = request, res = response) => {
    ...
    try {
        ...
        if (tempItem) {
            ...
            const deleteStatus = await eliminarImagen(`portafolio/img`, picture)
            .catch(err => {
                console.log(err)
                throw new Error(`Error at Line 82: ${err}`)
            })
